# ERP API - ERP Distribution

Guide complet pour lancer, tester et comprendre l'API ERP.

## 0) Positionnement conceptuel: ERP, OLTP et SOA

Ce dossier `erp-api/` correspond au **coeur ERP operationnel (OLTP)** du projet.
...
Ce perimetre couvre la couche transactionnelle. La couche decisionnelle est dans:

- `../BI/` = ETL + Data Warehouse (schema etoile)
- `../data_mining/` = analyses avancees
- `../ai-reporting/` = reporting assiste par IA
- `../interface_olap/` = interface web decisionnelle

## 1) Vue d'ensemble

L'ERP API contient 5 services Node.js:
...
1. Utiliser le `.env` unique a la racine du projet (`../.env`)
2. Ou copier `erp-api/.env.example` -> `erp-api/.env` pour un usage local
